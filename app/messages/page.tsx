...
import { Search, Video, Phone, MoreVertical, Send, Smile, Paperclip, UserPlus, Flag, MessageSquare, ArrowLeft, Trash2 } from 'lucide-react';
...
export default function ChatPage() {
    ...
    return (
        <div className="min-h-screen pt-16 md:pt-24 pb-0 md:pb-12 bg-transparent flex justify-center px-0 md:px-4 relative overflow-hidden">
            <ParticleEffect count={15} />

            <div className="relative w-full max-w-6xl h-[calc(100dvh-64px)] md:h-[85vh] group z-10 flex flex-col md:flex-row bg-white/70 dark:bg-[#0a0a0a]/70 backdrop-blur-3xl md:rounded-[32px] overflow-hidden shadow-2xl border-t md:border border-slate-200 dark:border-white/5">
                {/* Neon Glow Outer */}
                <div className="hidden md:block absolute -inset-1 bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 rounded-[32px] opacity-20 dark:opacity-30 group-hover:opacity-60 blur-xl transition duration-700 pointer-events-none" />
                ...
                {/* Chat Area */}
                <div className={`${isMobile && !activeChat ? 'hidden' : 'flex'} flex-1 flex-col bg-white/50 dark:bg-transparent h-full`}>
                    {activeChat ? (
                        <>
                            {/* Chat Header */}
                            ...
                            {/* Input Area */}
                            <div className="p-3 md:p-6 border-t border-slate-200 dark:border-white/5 bg-white/60 dark:bg-black/40 backdrop-blur-md pb-safe">
                                <div className="flex items-center gap-1 md:gap-3">
                                    <button className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-full transition-colors text-slate-400 dark:text-muted-foreground shrink-0">
                                    ...
                                    <div className="flex-1 relative">
                                        <input
                                            type="text"
                                            placeholder="Message..."
                                            className="w-full bg-slate-200/50 dark:bg-white/5 border-none rounded-full py-2.5 md:py-4 pl-4 pr-10 text-sm md:text-base focus:ring-1 focus:ring-primary/50 text-slate-900 dark:text-white transition-all"
                                        />
                                        <button className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded-full text-slate-400 dark:text-muted-foreground">
                                            <Smile className="w-4 h-4 md:w-5 md:h-5" />
                                        </button>
                                    </div>
                                    <button className="p-3 md:p-4 bg-primary rounded-full hover:opacity-90 transition-opacity text-primary-foreground shadow-lg shadow-primary/20 flex-shrink-0">
